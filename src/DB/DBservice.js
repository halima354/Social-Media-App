...
export const findOneAndUpdate= async ({model, data={},options={}, filter={}, select= "", populate= []}={})=>{
...
export const findByIdAndUpdate= async ({model, id="", data ={} ,options={},  select= "", populate= []}={})=>{
    const document = await model.findOneAndUpdate(id , data,options).select(select).populate(populate) 
    return document
}

export const updateOne= async ({model, data={},options={}, filter={} }={})=>{
    const document = await model.updateOne(filter,data,options)
    return document
}

export const updateMany= async ({model, data={},options={}, filter={} }={})=>{
